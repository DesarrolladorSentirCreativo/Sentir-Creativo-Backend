...
public class EstadoUserAdminConfiguration : IEntityTypeConfiguration<EstadoUserAdmin>
{
    public void Configure(EntityTypeBuilder<EstadoUserAdmin> builder)
    {
        builder.ToTable("estados_useradmins");
                        
        builder.HasKey(p => p.Id).HasName("id");
        builder.Property(p => p.Nombre).HasColumnName("nombre").IsRequired().HasMaxLength(80);
        builder.Property(p => p.Color).HasColumnName("color").IsRequired().HasMaxLength(8);
