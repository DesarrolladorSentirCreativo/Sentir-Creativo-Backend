...
public class ColeccionUserAdminConfiguration : IEntityTypeConfiguration<ColeccionUserAdmin>
{
    public void Configure(EntityTypeBuilder<ColeccionUserAdmin> builder)
    {
        builder.ToTable("colecciones");
                        
        builder.HasKey(p => p.Id).HasName("id");
        builder.Property(p => p.Nombre).HasColumnName("nombre").IsRequired().HasMaxLength(50);
        builder.Property(p => p.ModuloId).HasColumnName("modulo").IsRequired();
        ...
        builder.HasOne(u => u.Modulo)
            .WithOne()
            .HasForeignKey<ColeccionUserAdmin>("ModuloId");
    }
}